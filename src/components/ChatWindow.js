...
import { Pin, X, AlertCircle, CheckCircle2, Search, Star, MoreVertical, ArrowDown, BarChart2 } from 'lucide-react'; 
...
export default function ChatWindow({ activeGroup, currentUser, userData }) {
  const {
    ...
    handleCreatePoll, // Ensure this is exported from useChatLogic
  ...
  const handleScroll = () => {
    ...
    // If we hit bottom, clear the unread bubble
  ...
  // If useChatLogic doesn't fully handle the object creation, this bridge function ensures it works.
  const onPollSubmit = (pollData) => {
      // Pass the raw data to the logic hook
  ...
  const renderMessages = () => {
    ...
    return filtered.map((msg, index) => {
        ...
        if (!hasShownUnreadSeparator && !isSearching && !showStarredOnly && lastViewed > 0) {
            // If this message is newer than lastViewed, and it's not mine
    ...
    <div className="flex flex-col h-full bg-[#E3F2FD] relative overflow-hidden rounded-3xl shadow-xl border border-white/50">
      ...
      <div className="bg-white/90 backdrop-blur-md px-4 py-3 flex justify-between items-center shadow-sm z-30 border-b border-gray-100 relative h-16">
        <div className="flex-1 relative h-full flex items-center overflow-hidden">
            <div className={`absolute left-0 w-full transition-all duration-300 ease-in-out transform ${isSearching ? 'translate-y-12 opacity-0' : 'translate-y-0 opacity-100'}`}>
        ...
        <div className="flex items-center gap-1 ml-3 shrink-0">
            ...
            {isManager && !isSearching && <button onClick={toggleRestriction} className="ml-1 p-2.5 hover:bg-gray-100 rounded-full text-gray-600 transition"><MoreVertical size={20} /></button>}
        </div>
      </div>
      ...
      <CreatePollModal 
        ...
        onCreate={onPollSubmit} // Uses the local wrapper
      />
